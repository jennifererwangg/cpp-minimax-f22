...
template<typename GameState>
std::pair<Game<GameState>, int> minimize(Game<GameState> game, int depth, int alpha, int beta, int maxDepth) {
    if (depth == maxDepth) {
        return game.eval();
    }
    ...
    for (Game<GameState> child : game.getNextPossibleMoves()) {
        std::pair<Game<GameState>, int> utility = maximize(child, alpha, beta, maxDepth);
